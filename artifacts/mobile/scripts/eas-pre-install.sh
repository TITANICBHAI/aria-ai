...
# ── Task 5: expo-gradle-plugin (install + patch) ─────────────────────────────
#
# expo-modules-core/android/build.gradle imports ExpoModuleExtension from the
# expo-gradle-plugin composite build. Without an includeBuild() pointing to it,
# Gradle cannot resolve the class and the build fails.
#
# The expo-gradle-plugin lives inside expo-modules-autolinking. We copy it to
# android/node_modules/expo-gradle-plugin so settings.gradle can find it at
# priority-1 (before pnpm install has run) and also patch its settings.gradle.kts
# to remove the pluginManagement block (same reason as @react-native/gradle-plugin).
#
EXPO_AUTOLINKING_VERSION="3.0.24"
EXPO_GRADLE_PLUGIN_DIR="android/node_modules/expo-gradle-plugin"

echo "=== ARIA EAS pre-install: checking expo-gradle-plugin ==="

if [ ! -d "$EXPO_GRADLE_PLUGIN_DIR" ]; then
    echo "Installing expo-modules-autolinking@${EXPO_AUTOLINKING_VERSION} to extract expo-gradle-plugin…"

    EXPO_AUTOLINKING_TMP=$(mktemp -d)

    if npm install \
        --prefix "$EXPO_AUTOLINKING_TMP" \
        --no-save \
        "expo-modules-autolinking@${EXPO_AUTOLINKING_VERSION}" 2>&1; then

        EXPO_PLUGIN_SRC="$EXPO_AUTOLINKING_TMP/node_modules/expo-modules-autolinking/android/expo-gradle-plugin"
        if [ -d "$EXPO_PLUGIN_SRC" ]; then
            mkdir -p "$(dirname "$EXPO_GRADLE_PLUGIN_DIR")"
            cp -r "$EXPO_PLUGIN_SRC" "$EXPO_GRADLE_PLUGIN_DIR"
            echo "=== expo-gradle-plugin installed at $EXPO_GRADLE_PLUGIN_DIR ==="
        else
            echo "WARNING: expo-gradle-plugin not found in expo-modules-autolinking package"
        fi
    else
        echo "WARNING: npm install for expo-modules-autolinking failed"
    fi

    rm -rf "$EXPO_AUTOLINKING_TMP"
else
    echo "expo-gradle-plugin already in android/node_modules/"
fi

# ── Task 5b: Patch expo-gradle-plugin settings to remove pluginManagement ────
EXPO_PLUGIN_SETTINGS="$EXPO_GRADLE_PLUGIN_DIR/settings.gradle.kts"

echo "=== ARIA EAS pre-install: patching expo-gradle-plugin settings ==="

if grep -q "pluginManagement" "$EXPO_PLUGIN_SETTINGS" 2>/dev/null; then
    node -e "
const fs = require('fs');
const path = '$EXPO_PLUGIN_SETTINGS';
let text = fs.readFileSync(path, 'utf8');
...
console.log('Removed pluginManagement block from ' + path);
"
    echo "=== Patched $EXPO_PLUGIN_SETTINGS: removed pluginManagement block ==="
else
    echo "=== $EXPO_PLUGIN_SETTINGS already has no pluginManagement block ==="
fi
